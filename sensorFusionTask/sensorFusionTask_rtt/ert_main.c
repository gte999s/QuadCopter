...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(sensorFusionTask_M) == (NULL)) &&
         !rtmGetStopRequested(sensorFusionTask_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(sensorFusionTask_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(sensorFusionTask_M, true);
      }

      if (rtmGetStopRequested(sensorFusionTask_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(sensorFusionTask_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(sensorFusionTask_M, true);
      }
    }

    sensorFusionTask_output();
    ...
    {                                  /* Sample time: [0.001s, 0.0s] */
      rtExtModeUpload(0, sensorFusionTask_M->Timing.taskTime0);
    }

    sensorFusionTask_update();
...
void MW_exitHandler(int sig)
{
  rtmSetErrorStatus(sensorFusionTask_M, "stopping the model");
  sem_post(&stopSem);
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(sensorFusionTask_M, 0);
  ...
  sensorFusionTask_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(sensorFusionTask_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(sensorFusionTask_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(sensorFusionTask_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(sensorFusionTask_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(sensorFusionTask_M));
  }
