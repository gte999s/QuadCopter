...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(udpRead_M) == (NULL)) && !rtmGetStopRequested
         (udpRead_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(udpRead_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(udpRead_M, true);
      }

      if (rtmGetStopRequested(udpRead_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(udpRead_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(udpRead_M, true);
      }
    }
    ...
    {                                  /* Sample time: [0.001s, 0.0s] */
      rtExtModeUpload(0, udpRead_M->Timing.taskTime0);
    }
...
void MW_exitHandler(int sig)
{
  rtmSetErrorStatus(udpRead_M, "stopping the model");
  sem_post(&stopSem);
}
...
int main(int argc, char **argv)
{
  ...
  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(udpRead_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(udpRead_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(udpRead_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(udpRead_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(udpRead_M));
  }
