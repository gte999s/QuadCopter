...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(readRegisterTest_M) == (NULL)) &&
         !rtmGetStopRequested(readRegisterTest_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(readRegisterTest_M->extModeInfo, 2, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(readRegisterTest_M, true);
      }

      if (rtmGetStopRequested(readRegisterTest_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(readRegisterTest_M->extModeInfo, 2, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(readRegisterTest_M, true);
      }
    }

    readRegisterTest_output();
    ...
    {                                  /* Sample time: [0.0s, 0.0s] */
      rtExtModeUpload(0, readRegisterTest_M->Timing.t[0]);
    }

    {                                  /* Sample time: [0.01s, 0.0s] */
      rtExtModeUpload(1, ((readRegisterTest_M->Timing.clockTick1) * 0.01));
    }

    readRegisterTest_update();
...
void MW_exitHandler(int sig)
{
  rtmSetErrorStatus(readRegisterTest_M, "stopping the model");
  sem_post(&stopSem);
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(readRegisterTest_M, 0);
  ...
  readRegisterTest_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(readRegisterTest_M));
  rtExtModeCheckInit(2);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(readRegisterTest_M->extModeInfo, 2, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(readRegisterTest_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(readRegisterTest_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(readRegisterTest_M));
  }
