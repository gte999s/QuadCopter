...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(MPU6050_Gyro_Accel_M) == (NULL)) &&
         !rtmGetStopRequested(MPU6050_Gyro_Accel_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(MPU6050_Gyro_Accel_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(MPU6050_Gyro_Accel_M, true);
      }

      if (rtmGetStopRequested(MPU6050_Gyro_Accel_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(MPU6050_Gyro_Accel_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(MPU6050_Gyro_Accel_M, true);
      }
    }

    MPU6050_Gyro_Accel_output();
    ...
    {                                  /* Sample time: [0.1s, 0.0s] */
      rtExtModeUpload(0, MPU6050_Gyro_Accel_M->Timing.taskTime0);
    }

    MPU6050_Gyro_Accel_update();
...
void MW_exitHandler(int sig)
{
  rtmSetErrorStatus(MPU6050_Gyro_Accel_M, "stopping the model");
  sem_post(&stopSem);
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(MPU6050_Gyro_Accel_M, 0);
  ...
  MPU6050_Gyro_Accel_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(MPU6050_Gyro_Accel_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(MPU6050_Gyro_Accel_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(MPU6050_Gyro_Accel_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(MPU6050_Gyro_Accel_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(MPU6050_Gyro_Accel_M));
  }
